...
def find_mul_instructions():
    """Finds all the valid mul instructions in the file"""
    instructions = read_file()
    # match mul(X,Y) where X and Y are 1 to 3 digits
    pattern = r"mul\((\d{1,3}),(\d{1,3})\)" # use regex to find real instructions
    matches = re.findall(pattern, instructions)
    return matches

def find_mul_results(mul_instructions):
    """Finds the results of the mul instructions"""
    total = 0
    # multiply all x,y in uncorrupted instructions
...
def main():
    """Main function"""
    mul_instructions = find_mul_instructions()
    return find_mul_results(mul_instructions)
